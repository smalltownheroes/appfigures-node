...
class Client {
	...
	getDownloads(productIds, start_date, end_date, cb) {
		...
		this.get(`/reports/sales/?products=${products}&tz=utc&group_by=date&start_date=${start_date}&end_date=${end_date}`, cb);
	}

	getUsage(productIds, start_date, end_date, cb) {
		const products = productIds.join(';');
		this.get(`/reports/usage/?storefront=google_analytics&products=${products}&tz=utc&group_by=date&start_date=${start_date}&end_date=${end_date}`, cb);
	}

	getDailyRankings(productIds, start_date, end_date, cb) {
		const products = productIds.join(';');
		const countries = this.config.countries || 'be';
		this.get(`/ranks/${products}/daily/${start_date}/${end_date}/?countries=${countries}&tz=utc`, cb);
	}
}
